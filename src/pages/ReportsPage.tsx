        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div className="brand-outline-card rounded-[16px] p-4">
              ...
              <p className="mt-2 font-display text-[24px] text-[#1535D4]">{formatCurrency(revenue)}</p>
            ...
            <div className="brand-outline-card rounded-[16px] p-4">
              ...
              <p className="mt-2 font-display text-[24px] text-slate-900">{formatCurrency(commission)}</p>
            ...
            <div className="brand-outline-card rounded-[16px] p-4">
              ...
              <p className="mt-2 font-display text-[24px] text-slate-900">{formatCurrency(payBreakdown.cash)}</p>
            </div>
            <div className="brand-outline-card rounded-[16px] p-4">
              <p className="text-[11px] uppercase tracking-[0.16em] text-slate-500">QRIS / Transfer</p>
              <p className="mt-2 font-display text-[24px] text-slate-900">{formatCurrency(payBreakdown.qris + payBreakdown.transfer)}</p>
            </div>
          </div>
            ...
            {technicianRows.length === 0 ? (
              ...
              technicianRows.map((row) => (
                <div key={row.key} className="grid grid-cols-[1.2fr_0.6fr_0.8fr_0.8fr] items-center gap-2 px-4 py-4 text-sm">
                  ...
                  <div className="text-right text-slate-700">{row.count}</div>
                  <div className="text-right font-medium text-slate-900">{formatCurrency(row.revenue)}</div>
                  <div className="text-right font-medium text-[#1535D4]">{formatCurrency(row.commission)}</div>
                </div>
              ))
            )}
          </div>
